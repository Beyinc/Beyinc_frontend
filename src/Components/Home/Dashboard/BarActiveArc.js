...
export default function BarActiveArc({ Tdata }) {
  ...
  React.useEffect(() => {
    ...
    if (Object.keys(Tdata).length > 0) {
      ...
      Object.keys(Tdata?.connections).length > 0 &&
        Object.keys(Tdata?.connections)?.map((obj) => {
  ...
  return (
    <div>
      <div style={{ padding: "5px 25px" }} className="HeadingHolder">
        Connection Summary
      </div>
      {/* here create a component and add here the lists*/}
      {/* create and insert new component here */}
      <VerifiedUsersList />
