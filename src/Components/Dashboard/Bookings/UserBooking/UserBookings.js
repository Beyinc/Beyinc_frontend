...
//         console.log("User Booking Requests:", res);
...
//                     requestId={selectedBooking ? selectedBooking._id : null}
//                 />
//             </div>
//         </Dialog>
...
import BookSession from "../../../Editprofile/BookSession/BookSession2";
import dayjs from "dayjs";
import { VideoCameraFront, MoreVert } from "@mui/icons-material";
...
const RequestListItem = ({ req, isSelected, onSelect }) => {
  ...
  if (req.requestStatus === false) {
    statusLabel = "Pending";
    statusClass = "bg-yellow-100 text-yellow-700";
  } else if (req.requestStatus === true && req.booked === false) {
    statusLabel = "Approved";
    statusClass = "bg-blue-100 text-blue-700";
  } else if (req.requestStatus === true && req.booked === true) {
          ...
          <div className="flex items-center justify-between mb-1">
            <p
              className={`font-bold text-sm ${
                isSelected ? "text-[#4f55c7]" : "text-gray-900"
              }`}
            >
              {req.mentorId?.userName}
            </p>
            ...
            <span
              className={`text-[10px] px-2 py-1 rounded-md font-bold uppercase tracking-wider ${statusClass}`}
            >
              {statusLabel}
            </span>
          </div>
      ...
      <div className="flex items-center gap-4 flex-shrink-0 min-w-[200px]">
        ...
        <div>
          <p className="font-bold text-gray-900">
            {booking.mentorId?.userName || "Unknown Mentor"}
          </p>
          <p className="text-sm text-gray-600 truncate max-w-[150px]">
            {booking.mentorId?.email || "Individual"}
          </p>
...
const UserBooking = () => {
  ...
  useEffect(() => {
    const fetchUserRequests = async () => {
      try {
        const res = await CalendarServices.userBookingRequest();
        setUserBookingRequest(res.pendingRequests);
      } catch (err) {
        console.error(err);
      }
  ...
  const handleCloseDialog = (type) => {
    if (type === "reschedule") setOpenRescheduleDialog(false);
    else if (type === "cancel") setOpenCancelDialog(false);
  };
  const handleMenuClick = (event, booking) => {
    setAnchorEl(event.currentTarget);
    setSelectedBooking(booking);
  ...
  const handleJoinCall = (url) => window.open(url, "_blank");
  const handleContinueBooking = (req) => {
    setShowBookSession(true);
    setSelectedReq(req);
  };
...
const handleConfirmReschedule = async () => {
  try {
    const response = await CalendarServices.rescheduleBooking({
      bookingId: selectedBooking._id,
      newDate: selectedBooking.newDate,
    });
  ...
  return (
    <div className="bg-gray-50 min-h-screen p-4 md:p-8">
      <div className="max-w-[1400px] mx-auto">
        <h1 className="text-2xl font-bold text-gray-900 mb-6 font-roboto">
          User Bookings
        </h1>
        ...
        {tabValue === 2 &&
          (rescheduledBookings.length > 0 ? (
            ...
            <p className="text-gray-500">No rescheduled bookings.</p>
          ))}
        ...
        {/* {tabValue === 3 && (
          <div className="flex flex-col lg:flex-row gap-6 relative items-start animate-in fade-in duration-300">
            <div className="w-full lg:w-4/12">
              {userBookingRequest.length > 0 ? userBookingRequest.map(req => (
                <div
                  key={req._id}
                  onClick={() => handleContinueBooking(req)}
                  className={`relative p-4 cursor-pointer transition-all duration-200 rounded-xl mb-3 border ${selectedReq?._id === req._id ? "bg-white border-[#4f55c7] shadow-[0_0_0_1px_#4f55c7]" : "bg-white border-transparent hover:border-gray-200 hover:shadow-md hover:translate-x-1"}`}
                >
                  <p className="font-bold">{req.mentorId?.userName}</p>
                  <p>{req.requestStatus ? "Approved" : "Pending"}</p>
                </div>
        ...
        {tabValue === 3 && (
          <div className="flex flex-col lg:flex-row gap-6 relative items-start animate-in fade-in duration-300">
            ...
            <div className="w-full lg:w-12/12">
              {userBookingRequest && userBookingRequest.length > 0 ? (
                userBookingRequest
                  .filter((req) => req.requestStatus === true) // Only Pending
                  .map((req) => (
                    <RequestListItem
                      key={req._id}
                      req={req}
            ...
            <div className="w-full lg:w-8/12">
              {selectedReq ? (
                <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm sticky top-6">
                  {selectedReq.requestStatus ? (
                    selectedReq.booked === false ? (
                      // Approved & not booked → show BookSession
                      <div className="BookSessionCardWrapper">
                        <div className="flex justify-between items-center mb-4">
                      ...
                      // Approved & booked → show scheduled message
                      <div className="bg-white rounded-xl shadow-md border p-6 animate-in fade-in duration-300">
                        {/* Title */}
                        ...
                        <div className="flex items-center gap-4 mb-6">
                          <div className="w-14 h-14 rounded-full bg-gray-100 flex items-center justify-center text-[#4f55c7] font-bold text-xl border">
                            {selectedReq?.mentorId?.userName?.charAt(0)}
                          </div>
                        ...
                        <div className="space-y-4 bg-gray-50 p-4 rounded-lg border">
                          <Item label="Title">{selectedReq.title}</Item>
                          <Item label="Message">
                            "{selectedReq.requestMessage}"
                          </Item>

                          <Item label="Duration">
                            {selectedReq.duration} mins
                          </Item>
                          <Item label="Amount">
                            {selectedReq.amount} Rupees
                          </Item>

                          <Item label="Requested On">
                          ...
                          <div
                            className={`text-[20px] px-20 py-1 rounded-md font-semibold w-auto h-8  tracking-wider bg-green-100 text-green-700`}
                          >
                            Scheduled & Paid
                          </div>
                        </div>
                    ...
                    // Not approved → pending message
                    <div className="p-8 bg-gray-50 rounded-lg text-center border border-gray-200">
                      <p className="text-gray-500 italic">
                        This request is still pending approval from the mentor.
                      </p>
                    </div>
                  )}
                </div>
        ...
        {tabValue === 5 && (
          <div className="flex flex-col lg:flex-row gap-6 relative items-start animate-in fade-in duration-300">
            {/* Left Panel: Pending Requests List */}
            <div className="w-full lg:w-12/12">
              {userBookingRequest && userBookingRequest.length > 0 ? (
                userBookingRequest
                  .filter((req) => req.requestStatus === false) // Only Pending
                  .map((req) => (
                    <RequestListItem
                      key={req._id}
                      req={req}
            ...
            <div className="w-full lg:w-8/12 sticky top-24">
              {selectedReq ? (
                <div className="bg-white rounded-xl shadow-md border p-6 animate-in fade-in duration-300">
                  {/* Title */}
                  <h2 className="text-lg font-bold text-gray-900 mb-4">
                  ...
                  <div className="space-y-4 bg-gray-50 p-4 rounded-lg border">
                    ...
                    <Item label="Message">"{selectedReq.requestMessage}"</Item>

                    <Item label="Duration">{selectedReq.duration} mins</Item>
                    <Item label="Amount">{selectedReq.amount} Rupees</Item>
                    ...
                    <div
                      className={`text-[20px] px-32 py-1 rounded-md font-semibold w-auto h-8  tracking-wider bg-yellow-100 text-yellow-700`}
                    >
                      Pending Approval
                    </div>
                  </div>
