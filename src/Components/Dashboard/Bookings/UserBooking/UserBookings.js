...
import { Box, Typography, Tabs, Tab, Divider, List, ListItem, ListItemText, Button, Menu, MenuItem, Dialog, DialogTitle, DialogContent, DialogActions, TextField } from '@mui/material';
...
const UserBooking = () => {
    ...
    const fetchBookings = async () => {
      try {
        ...
        const upcomingBookings = userBookings.filter(booking => 
          new Date(booking.startDateTime) > now && booking.status === 'upcoming'
        );

        const completedBookings = userBookings.filter(booking => 
          new Date(booking.startDateTime) <= now && booking.status === 'completed'
        );

        const rescheduledBookings = userBookings.filter(booking => booking.mentorReschedule[0] === true);
  ...
  const handleConfirmCancel = async () => {
    ...
    try {
      ...
      const response = await CalendarServices.cancelBooking({selectedBooking})
          ...
          {tabValue === 0 && (
            <>
              ...
              <Box>
                ...
                <Box key={booking._id} border="1px solid black" borderRadius={1}  mb={3}>
                  {/* Booking Date */}
                  ...
                  <Box display="flex" flexDirection="row" alignItems="center">
                    ...
                    <Box width="20%" ml={2}>
                      <Typography variant="body1">
                        Mentor: {booking.mentorId ? booking.mentorId.userName : 'N/A'}
                      </Typography>
                    </Box>
                    <Box width="20%" ml={15}>
                    ...
                    {/* Display Duration */}
                    <Typography variant="body1">
                      Duration: {booking.duration ? `${booking.duration} mins` : 'N/A'}
                    </Typography>
                    ...
                    <Box width="20%" ml={5}>
                      <button className='joinCall'  onClick={() => handleJoinCall(booking.meetLink)}>
                        Join Call
                      </button>
                    </Box>
        ...
        <List>
          {completedBookings.map((booking) => (
            <Box key={booking._id} mb={3}>
               ...
               <Box display="flex" flexDirection="row" alignItems="center" border="1px solid black" borderRadius={1}>
                {/* 1st Column: Purple Vertical Box */}
                <Box width="1.5%" bgcolor="#4F55C7" height="100px" />
                ...
                <Box width="20%" ml={5}>
                  <button className='joinCall' onClick={() => handleTestimonial(booking, "feedback")} color="primary">
                    Give Feedback
                  </button>
                </Box>
              </Box>
  ...
  {tabValue === 2 && (
    <>
      ...
      {rescheduledBookings.length > 0 ? (
            ...
            <Box key={booking._id} mb={3}>
              ...
              <Box display="flex" flexDirection="row" alignItems="center" border="1px solid black" borderRadius={1}>
                ...
                <Box width="20%" ml={5}>
                  ...
                  <Typography variant="body1">
                   {booking.mentorReschedule[1] || 'No reason provided'}
                  </Typography>
                </Box>
              </Box>
            </Box>
        ...
        >
          <MenuItem onClick={() => handleOpenDialog(selectedBooking, 'reschedule')}>Reschedule</MenuItem>
          <MenuItem onClick={() => handleOpenDialog(selectedBooking, 'cancel')}>Cancel</MenuItem>
        </Menu>

         {/* Reschedule Calendar Modal */}
        {/* Reschedule Modal */}
        {actionType === 'reschedule' && (
          <Dialog open={openRescheduleDialog} onClose={() => handleCloseDialog('reschedule')}>
            <div>
              <h1>Reschedule</h1>
              <BookSession
